...
elif [[ -z "${CIRCLE_PR_NUMBER}" && "$CIRCLE_BRANCH" == "main" ]]; then
  echo "On main branch, running clang-tidy-diff against previous commit..."
  git diff HEAD^ | filter_excludes | "${LLVM_PREFIX}/share/clang/clang-tidy-diff.py" -p 1
else
  echo "Running clang-tidy-diff against main branch..."
  git fetch https://github.com/envoyproxy/envoy.git main
